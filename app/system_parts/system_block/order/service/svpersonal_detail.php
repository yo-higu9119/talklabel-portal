...
?>
				<div class="comBox order_Box">
					...
					<form method="post" action="<?php $path_parts = pathinfo($_SERVER["HTTP_HOST"].$_SERVER["REQUEST_URI"]);echo $path_parts['basename'];?>" id="act">
						<input type="hidden" name="si" value="<?php echo $itemId;?>" />
						<div class="BtM mglra clear_fix">
							<p><button type="button" class="whBT mglra mgt20 mgb10 btWtW back" onclick="$('#act').attr('action', '../../');$('#act').submit();" /><?php echo Util::dispLang(Language::WORD_00299);/* トップページへ進む */?></button></p>
						</div>
					</form>
				</div>
<?php
...
require dirname(__FILE__).'/../../../../system_parts/system_block/order/service/sv_personal_login.php';
?>
				<div class="comBox order_Box">
					<form method="post" action="<?php $path_parts = pathinfo($_SERVER["HTTP_HOST"].$_SERVER["REQUEST_URI"]);echo $path_parts['basename'];?>" id="act">
						<input type="hidden" name="mode" id="mode" value="save" />
						<input type="hidden" name="sv" value="<?php echo $itemId;?>" />
