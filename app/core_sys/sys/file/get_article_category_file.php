...
if(isset($_GET['id']) && is_numeric($_GET['id']) //ID
	...
	$info = $categoryData->getInfo($id);
	if($info->id === 0) {
		header('HTTP/1.0 404 Not Found');
		exit();
	}
	if(!isset($_SESSION['preview']['key']) && !$info->isDisp($session->isLogin(), $session->getMemberPurchased(), $session->getMemberId())) {
		header('HTTP/1.0 404 Not Found');
		exit();
	}

	$filePath = PathInfo::getCategoryFilePath(false, $id, 0);
	if(is_file($filePath)) {
