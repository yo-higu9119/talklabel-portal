...
&& isset($_GET['f'])	//ファイル名
	...
	switch(intval($_GET['type'])) {
		case 8:
			...
			break;
		case 12:
			$filePath = PathInfo::getProductFilePath(intval($_GET['r'])===2, $_GET['id'], $session->getMemberId());
			break;
		case 13:
			$filePath = PathInfo::getProductFilePath2(intval($_GET['r'])===2, $_GET['id'], $_GET['sub'], $session->getMemberId());
			break;
		case 19:
			$filePath = PathInfo::getInquiryFilePath(intval($_GET['r'])===2, $_GET['id'], $_GET['sub'], isset($_GET['add'])?$_GET['add']:0, false, false, isset($_GET['add'])?$_GET['add']:0);
			break;
		default:
			header('HTTP/1.0 404 Not Found');
			exit();
