	...
	//if($session->isLogin()){
		...
		if($data === NULL){
			...
			if($sErr == ""){
				...
				if($option_id > 0){
					/* cart option */
					if(isset($_SESSION[$cart_name_caop]) && is_array($_SESSION[$cart_name_caop]) && count($_SESSION[$cart_name_caop]) > 0){
						if(array_key_exists($option_id,$_SESSION[$cart_name_caop])){
							$_SESSION[$cart_name_caop][$option_id] = intval($data['num']);
						}else{
							$_SESSION[$cart_name_caop][$option_id] = intval($data['num']);
						}
					}else{
						$_SESSION[$cart_name_caop] = array();
						$_SESSION[$cart_name_caop][$option_id] = intval($data['num']);
					}
				}
				/*** session ***/
...
__LONG_STRRING__;
					...
					$ret["tags"] = '<div class="CautTxt CautMg nocartArt cnt">現在カートに商品はありません。<br>商品一覧から購入したい商品を<br />カートに入れてください。</div>';
					$ret["status"] = "none";
				}
