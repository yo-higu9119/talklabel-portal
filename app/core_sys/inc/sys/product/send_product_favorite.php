	...
	if($session->isLogin()){
		...
		if($data === NULL){
			...
			if($sErr == ""){
				...
				if($productInfo->id > 0){
					$Favorite = new ProductFavoriteData($session->getMemberName());
					if($Favorite->getIsCount($productInfo->id,$session->getMemberId()) > 0){
						if($Favorite->is_member_delete($productInfo->id,$session->getMemberId())){
							$ret["flg"] = 0;
							$ret["count"] = $Favorite->getFavoriteCount($productInfo->id);
							$ret["status"] = "success";
						}else{
							$ret["status"] = "error";
						}
					}else{
						$favoriteInfo = new ProductFavoriteInfo();
						$favoriteInfo->product_id         = $productInfo->id;
