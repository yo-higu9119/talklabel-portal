...
if(count($breadcrumbList) > 0){
	foreach($breadcrumbList as $breadcrumbInfo) {
		...
		if($auto_link == 0){
			$link_str = htmlspecialchars(SYSTEM_TOP_URL.$groupfileName.'list.php?c='.$categoryId.'&pc='.$id);
		}else if($auto_link == 1){
			if(count($contents) == 1){
				$link_str = htmlspecialchars(SYSTEM_TOP_URL.$groupfileName.'index.php?c='.$categoryId.'&pc='.$id.'&s='.$contents[0]);
			}else{
				$link_str = htmlspecialchars(SYSTEM_TOP_URL.$groupfileName.'list.php?c='.$categoryId.'&pc='.$id);
			}
		}else{
...
<?php
	...
	if(isset($_GET['s'])){
		...
		$panProductInfo = $productData->getInfoFromUrlKey($_GET['s']);
		if($panProductInfo->id > 0 &&  $panProductInfo->isOpen) {
?>
					<li>｜</li>
					<li><?php echo htmlspecialchars($panProductInfo->name)?></li>
<?php
		}
