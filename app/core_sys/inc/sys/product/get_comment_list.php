...
function getTags($info,$class,$session,$prohibition,$systemInfo){
	...
	if($info->status == 2){
		...
		$commentRespond = new ProductCommentRespondData($session->getMemberName());
		...
		if($info->member_id == $session->getMemberId()){
			...
			$str .= '</div>';
			$str .= '<div class="commentResR clear_fix">';
			$commentReportData = new ProductCommentReportData($session->getMemberName());
	...
	if($info->status == 2){
		...
		if($info->member_id == $session->getMemberId()){
			...
			$str .= '<p class="commentForm"><textarea name="input_comment'.$info->id.'" cols="50" rows="5"  class="txt size100p"  placeholder="'.Util::dispLang(Language::WORD_00652).'" >'.htmlspecialchars($info->body).'</textarea></p>';/* レビューを入力してください */
			$str .= '</div>';
			...
			$str .= '<p class="CautTxt CautMg cnt">'.Util::dispLang(Language::WORD_00667).'</p>';/* このレビューを削除しますか？ */
			$str .= '<div class="clear_fix">';
			$str .= '<p class="commentBT BtM"><button type="button" class="commentDelBt" onclick="utilOpenFrame(\'../../popup/product_comment/comment_dialog.php?id='.$info->id.'&t=de\', false, '.$utilOpenFrame_prm.');" >'.Util::dispLang(Language::WORD_00024).'</button></p>';/* 削除する */
			$str .= '</div>';
			$str .= '</div>';
		}else{
			...
			$str .= '<div class="commentInput">';
			$str .= '<p class="commentForm">'.FormUtil::textbox('input_nickname'.$info->id, htmlspecialchars($session->getMemberNickname()), 10, 250, 'txt size100p', Util::dispLang(Language::WORD_00018)).'</p>';/* ニックネームを入れてください */
			$str .= '<p class="commentForm"><textarea name="input_comment'.$info->id.'" cols="50" rows="5"  class="txt size100p"  placeholder="'.Util::dispLang(Language::WORD_00652).'" ></textarea></p>';/* レビューを入力してください */
			$str .= '</div>';
			$str .= '<div class="clear_fix">';
			$str .= '<p class="commentBT BtM"><button type="button" class="commentResBt" onclick="comment_check('.$info->id.');" >'.Util::dispLang(Language::WORD_00668).'</button></p>';/* 返信のレビューする */
			$str .= '</div>';
			$str .= '</div>';
		...
		if($data === NULL){
			...
			if($sErr == ""){
				...
				if($systemInfo->app_login_type == 0 || ($systemInfo->app_login_type == 1 && $session->isLogin())){
					...
					$totalCnt = $productCommentData->getListPrCnt($data['product_id']);
					$List = $productCommentData->getInfoListPr($data['product_id'],$pageNo*$pageDispCntMax,$pageDispCntMax);
