	...
	if($session->isLogin()){
		...
		if($data === NULL){
			...
			if($sErr == ""){
				...
				$commentData = new ProductCommentData($session->getMemberName());
				...
				if($commentInfo->id > 0){
					...
					if($commentReportData->getIsCount($commentInfo->id,$session->getMemberId()) == 0){
						$commentRepInfo = new ProductCommentReportInfo();
						$commentRepInfo->product_id         = $commentInfo->product_id;
						$commentRepInfo->product_comment_id = $commentInfo->id;
