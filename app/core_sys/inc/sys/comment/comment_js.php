<?php
if(isset($info->comment_setting) && $info->comment_setting == 1){
?>
<script>
var _nowPage = 1;
...
function comment_check(pid) {
	...
	if(body_obj.val().replace(/\s+/g, "") == ''){
		$(".ComErrDisp").remove();
		body_obj.before('<p class="Art cnt CautMg ComErrDisp"><?php echo Util::dispLang(Language::WORD_00056);/* コメントが入力されていません */?></p>');
		return false;
	}else{
		$(".ComErrDisp").remove();
<?php if (IS_SMART_PHONE) { ?>
		utilOpenFrame('../../popup/comment/comment_dialog.php?id='+pid+'&t=rs', false, 50, 340, 400);
<?php } else { ?>
		utilOpenFrame('../../popup/comment/comment_dialog.php?id='+pid+'&t=rs', false, 50, 700, 500);
<?php } ?>
	}
}
function comment_submit(pid) {
	...
	var nickname = nickname_obj.val();
	 $.ajax({
	 	type: "post",
	 	url: '<?php echo SYSTEM_TOP_URL; ?>core_sys/inc/sys/contents/send_comment.php',
		data: JSON.stringify({"article_id" : article_id, "parent_id" : parent_id, "body" : body, "nickname" : nickname }),
...
function comment_edit_check(pid) {
	...
	if(body_obj.val().replace(/\s+/g, "") == ''){
		...
		$(".ComErrDisp").remove();
<?php if (IS_SMART_PHONE) { ?>
		utilOpenFrame('../../popup/comment/comment_dialog.php?id='+pid+'&t=ed', false, 50, 340, 400);
<?php } else { ?>
		utilOpenFrame('../../popup/comment/comment_dialog.php?id='+pid+'&t=ed', false, 50, 700, 500);
<?php } ?>
	}
}
function comment_edit_submit(pid) {
	...
	var nickname = nickname_obj.val();
	 $.ajax({
	 	type: "post",
	 	url: '<?php echo SYSTEM_TOP_URL; ?>core_sys/inc/sys/contents/send_edit_comment.php',
...
function comment_rep_check(pid) {
	...
	if(body_obj.val().replace(/\s+/g, "") == ''){
		$(".ComErrDisp").remove();
		body_obj.before('<p class="Art cnt CautMg ComErrDisp"><?php echo Util::dispLang(Language::WORD_00057);/* 通報内容が入力されていません */?></p>');
		return false;
	}else{
		$(".ComErrDisp").remove();
<?php if (IS_SMART_PHONE) { ?>
		utilOpenFrame('../../popup/comment/comment_dialog.php?id='+pid+'&t=rp', false, 50, 340, 400);
<?php } else { ?>
		utilOpenFrame('../../popup/comment/comment_dialog.php?id='+pid+'&t=rp', false, 50, 700, 500);
<?php } ?>
	}
}
function comment_rep_submit(pid) {
	...
	var body = body_obj.val();
	 $.ajax({
	 	type: "post",
	 	url: '<?php echo SYSTEM_TOP_URL; ?>core_sys/inc/sys/contents/send_rep_comment.php',
...
function comment_del_submit(pid) {
	if(pid == 0){
	 ...
	 $.ajax({
	 	type: "post",
	 	url: '<?php echo SYSTEM_TOP_URL; ?>core_sys/inc/sys/contents/send_del_comment.php',
...
function comment_empathy_submit(pid) {
	if(pid == 0){
	 ...
	 $.ajax({
	 	type: "post",
	 	url: '<?php echo SYSTEM_TOP_URL; ?>core_sys/inc/sys/contents/send_empathy_comment.php',
		data: JSON.stringify({"id" : pid }),
		contentType: 'application/json',
		dataType: "json",
		success: function (data1) {
...
function comment_get_all(page,loc) {
	...
	var article_id = <?php echo $info->id; ?>;
	 $.ajax({
	 	type: "post",
	 	url: '<?php echo SYSTEM_TOP_URL; ?>core_sys/inc/sys/contents/get_comment_list.php',
		data: JSON.stringify({"article_id" : article_id, "page" : page }),
		contentType: 'application/json',
		dataType: "json",
		success: function (data1) {
			$("#loading ").hide();
...
if(isset($info) && $info->niceDisp){
?>
<script>
function article_empathy_submit(pid) {
	if(pid == 0){
		return false;
	}
	 $.ajax({
	 	type: "post",
	 	url: '<?php echo SYSTEM_TOP_URL; ?>core_sys/inc/sys/contents/send_article_empathy.php',
		data: JSON.stringify({"id" : pid }),
		contentType: 'application/json',
		dataType: "json",
		success: function (data1) {
			if(data1.status == 'success'){
				if(data1.flg == 0){//共感していない
					$('.ArtShare'+pid).removeClass('crt');
				}else{//共感している
					$('.ArtShare'+pid).addClass('crt');
				}
				$('.ArtShareNam'+pid).text(data1.count);//現在の共感数
			}else{
				console.log('post error');
			}
		},
...
if(isset($info) && $info->favorite_disp){
?>
<script>
function article_favorite_submit(pid) {
	if(pid == 0){
		return false;
	}
	 $.ajax({
	 	type: "post",
	 	url: '<?php echo SYSTEM_TOP_URL; ?>core_sys/inc/sys/contents/send_article_favorite.php',
		data: JSON.stringify({"id" : pid }),
		contentType: 'application/json',
		dataType: "json",
		success: function (data1) {
			if(data1.status == 'success'){
				if(data1.flg == 0){//お気に入りしていない
					$('.ArtFavorite'+pid).removeClass('crt');
				}else{//お気に入りしている
					$('.ArtFavorite'+pid).addClass('crt');
				}
			}else{
				console.log('post error');
			}
