...
function getTags($info,$class,$session,$prohibition,$systemInfo){
	...
	if($systemInfo->common_id == 1 || $info->status == 2 || $info->status == 1 || $info->auth_status == 2){
		...
		$nickname = (trim($info->nickname) !== "")?$info->nickname:"-";
		$str .= '<p class="commentName">'.htmlspecialchars($nickname).'<span class="commentNameS">【 '.$referral_id.' 】</span></p>';
	}
	...
	if($info->status == 2){
		...
		if($info->member_id == $session->getMemberId()){
			...
			$str .= '<div class="clear_fix">';
			$str .= '<p class="commentBT BtM"><button type="button" class="commentDelBt" onclick="utilOpenFrame(\'../../popup/board/comment_dialog.php?id='.$info->id.'&t=de\', false, '.$utilOpenFrame_prm.');" >'.Util::dispLang(Language::WORD_00024).'</button></p>';/* 削除する */
			$str .= '</div>';
			$str .= '</div>';
		}else{
		...
		if($data === NULL){
			...
			if($sErr == ""){
				...
				if($systemInfo->app_login_type == 0 || ($systemInfo->app_login_type == 1 && $session->isLogin())){
					...
					$totalCnt = $boardCommentData->getListPrCnt($data['board_id']);
					$List = $boardCommentData->getInfoListPr($data['board_id'],$pageNo*$pageDispCntMax,$pageDispCntMax);
					...
					if(count($List) !== 0){
						...
						$ret["tags"] = '<p class="CautTxt mgt10 mgb10 cnt">'.Util::dispLang(Language::WORD_00151).'</button></p>';/* コメントは一件もありません */
						$ret["status"] = "success";
					}
				}else{
					$ret["status"] = "error";
