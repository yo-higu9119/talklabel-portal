	...
	if($session->isLogin()){
		...
		if($data === NULL){
			...
			if($sErr == ""){
				...
				$commentData = new ArticleCommentData($session->getMemberName());
				...
				if($commentInfo->id > 0){
					...
					if($commentReportData->getIsCount($commentInfo->id,$session->getMemberId()) == 0){
						$commentRepInfo = new ArticleCommentReportInfo();
						$commentRepInfo->article_id         = $commentInfo->article_id;
						$commentRepInfo->article_comment_id = $commentInfo->id;
