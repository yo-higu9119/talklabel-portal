	...
	if($session->isLogin()){
		...
		if($data === NULL){
			...
			if($sErr == ""){
				...
				$articleInfo = $articleData->getInfo(intval($data['article_id']));
				if($articleInfo->id > 0){
					...
					$commentInfo = new ArticleCommentInfo();
					$commentInfo->article_id  = intval($data['article_id']);
					...
					$commentInfo->auth_status = $articleInfo->commentAuthDefoult;
					$commentInfo->id = $articleCommentData->insert($commentInfo);
