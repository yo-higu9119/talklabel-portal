			...
			if($sErr == ""){
				if($itemInfo->type === 1){/* 1:都度 */
					...
					if($itemInfo->pay_timing === 1){/* 即時課金 */
						...
						if($systemInfo->payment_timing == 1){//月末
							$purchaseChInfo->pay_date = date("Y/m/t");
						}else if($systemInfo->payment_timing == 2){//月初
							$purchaseChInfo->pay_date = date("Y/m/1");
						}else if($systemInfo->payment_timing == 3){//前回決済から指定日数後
							$purchaseChInfo->pay_date = date("Y/m/d");
						}else{//4:毎月指定日
							$matubi = date("t",strtotime('first day of '.date("Y/m/d")));
							if($matubi < $systemInfo->payment_timing_day){
								$payment_timing_day = $matubi;
							}else{
								$payment_timing_day = $systemInfo->payment_timing_day;
							}
							$purchaseChInfo->pay_date = date("Y/m/".$payment_timing_day);
						}
						...
						for($i=1;$i<=$itemInfo->split;$i++){
							...
							if($i == 1){
								...
								if($systemInfo->payment_timing == 1){//月末
									$purchaseChInfo->pay_date = date("Y/m/t",strtotime(date("Y/m/d",strtotime('first day of '.date("Y/m/d"))).$add_month));
								}else if($systemInfo->payment_timing == 2){//月初
									$purchaseChInfo->pay_date = date("Y/m/d",strtotime(date("Y/m/d",strtotime('first day of '.date("Y/m/d"))).$add_month));
								}else if($systemInfo->payment_timing == 3){//前回決済から指定日数後
									$purchaseChInfo->pay_date = date("Y/m/d",strtotime(date("Y/m/d").'+'.($systemInfo->payment_timing_day*$i).' day'));
								}else{//4:毎月指定日
									$matubi = date("t",strtotime('first day of '.date("Y/m/d").$add_month));
									if($matubi < $systemInfo->payment_timing_day){
										$payment_timing_day = $matubi;
									}else{
										$payment_timing_day = $systemInfo->payment_timing_day;
									}
									$purchaseChInfo->pay_date = date("Y/m/".$payment_timing_day,strtotime('first day of '.date("Y/m/d").$add_month));
								}
